...
class Supervisor:
    ...
    def decide(self, task: Task, context: AnalysisContext, state: RuntimeState) -> RouteDecision:
        ...
        if current == AgentRoute.VERIFICATION.value:
            verification_messages = [message for message in context.messages if message.message_type.value == "verification_result"]
            retry = bool(verification_messages and verification_messages[-1].payload.get("request_additional_analysis", False))
            attempts = int(state["runtime_metadata"].get("analysis_retries", 0))
            if retry and attempts < self.max_analysis_retries:
                state["runtime_metadata"]["analysis_retries"] = attempts + 1
                route = AgentRoute.BINARY_ANALYSIS if task.target.target_type is TargetType.BINARY else AgentRoute.SOURCE_ANALYSIS
                return RouteDecision(route, "verification requested bounded additional analysis", metadata={"retry": True})
            return RouteDecision(AgentRoute.REVIEWER, "verification results require independent review")
